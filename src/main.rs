...
use crate::controller::{RenderEngine, Canvas};
use crate::shm::ShmListener;

fn print_usage(program_name: &str) {
    println!("Usage:");
    println!("  {} static <r> <g> <b> <m>  - Set static color and brightness", program_name);
    println!("  {} rainbow                 - Run rainbow effect (foreground)", program_name);
    println!("  {} wave                    - Run wave effect (foreground)", program_name);
    println!("  {} shm                     - Read from /tmp/feelinlight.bin (foreground)", program_name);
    println!("  {} off                     - Turn off", program_name);
    println!("\nExamples:");
    println!("  {} static 245 245 220 255  # Pleasant Beige", program_name);
}
...
async fn main() -> Result<()> {
    ...
    match args[1].as_str() {
        "static" => {
            ...
            println!("Set static color RGB({}, {}, {}) up to index {}", r, g, b, end);
        }
        "off" => {
            protocol.send_segment(0, 0, 0, 0)?;
            println!("Turned off");
        }
        "rainbow" | "wave" | "shm" => {
